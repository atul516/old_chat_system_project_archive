var textDiv = null;
 ...
 function assign_actions(from_user){
    handleTop = document.getElementById("handleTop" + from_user);
    textDiv = document.getElementById(from_user);
    var d = new dragObject(textDiv, handleTop, new Position(25, 25), new Position((screen.availWidth) - 100, (screen.availHeight) - 200), null, null, null, false);
}
 ...
 function absoluteCursorPostion(eventObj){
    eventObj = eventObj ? eventObj: window.event;
    if (isNaN(window.scrollX)) return new Position(eventObj.clientX + document.documentElement.scrollLeft + document.body.scrollLeft, 
       eventObj.clientY + document.documentElement.scrollTop + document.body.scrollTop);
    else return new Position(eventObj.clientX + window.scrollX, eventObj.clientY + window.scrollY);
}
 
 function dragObject(element, attachElement, lowerBound, upperBound, startCallback, moveCallback, endCallback, 
   attachLater){
    if (typeof (element) == "string") element = document.getElementById(element);
 ...
 function DoHeight(y, element){
    ...
    textBox.style.height = (y - 80) + 'px';
    textBox1.style.top = (y - 65) + 'px';
}
 ...
 function DoWidth(x, element)    {
    ...
    handleTop.style.width = (x + 5) + 'px';
    textBox.style.width = (x - 5) + 'px';
    textBox1.style.width = (x - 5) + 'px';
}
